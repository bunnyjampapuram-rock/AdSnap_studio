...
def wait_for_bria_result(status_url, api_key, timeout=180, poll_interval=3, status_container=None):
    """Poll a Bria v2 async job until the generated image is ready."""
    if not status_url:
        raise ValueError("Bria did not return a status_url.")
    ...
    while time.monotonic() < deadline:
        ...
        if response.status_code >= 400:
            raise RuntimeError(
                f"Bria status request failed with HTTP {response.status_code}: {data}"
            )
        ...
        if status == "COMPLETED":
            ...
            if isinstance(result, dict) and result.get("image_url"):
                return result
            raise RuntimeError(
                f"Bria marked the job completed but returned no image_url: {data}"
            )

        if status in ("ERROR", "FAILED", "UNKNOWN"):
            error = data.get("error", data)
            raise RuntimeError(f"Bria image generation failed: {error}")

        if status_container is not None:
            status_container.info(
                f"🎨 Bria is generating your image... (check {attempt})"
            )

        time.sleep(poll_interval)

    raise TimeoutError(
        f"Bria image generation did not finish within {timeout} seconds."
    )
...
def main():
    ...
    with tabs[0]:
        ...
        if st.button("🎨 Generate Images", type="primary"):
            ...
            try:
                # Bria v2 4MP generation is asynchronous.
                # sync=True + 4MP causes HTTP 422.
                for image_number in range(num_images):
                    status_container.info(
                        f"🎨 Submitting image {image_number + 1} of {num_images}..."
                    )

                    submit_result = generate_hd_image(
                    ...
                    if not isinstance(submit_result, dict):
                        raise RuntimeError(f"Unexpected Bria response: {submit_result}")

                    status_url = submit_result.get("status_url")
                    image_url = None

                    if status_url:
                        final_result = wait_for_bria_result(
                        ...
                        if isinstance(result_obj, dict):
                            image_url = result_obj.get("image_url")
                    if image_url:
                        ...
                        generated_urls.append(image)
                    else:
                        raise RuntimeError(
                            f"Bria completed without returning an image URL: {submit_result}"
                        )

                st.session_state.generated_images = generated_urls
                st.session_state.edited_image = generated_urls[0]
                ...
                if edit_option == "Create Packshot":
                    ...
                    if st.button("Create Packshot"):
                        with st.spinner("Creating professional packshot..."):
                            try:
                                # create_packshot() handles background removal via force_rmbg.
    ...
    with tabs[3]:
        ...
        if uploaded_file:
            ...
            with col2:
                if st.session_state.edited_image:
                    st.image(st.session_state.edited_image, caption="Result",   width="stretch")
                    image_data = download_image(st.session_state.edited_image)
                    if image_data:
                        st.download_button(
                            "⬇️ Download Result",
